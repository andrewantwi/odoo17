...
class HospitalAppointment(models.Model):
    ...
    patient_id = fields.Many2one('hospital.patient', string='Patient', ondelete='restrict')  # Links to Patient model
    date_appointment = fields.Date(string='Date')
    note = fields.Text(string='Note')
    state = fields.Selection([('draft', 'Draft'), ('confirmed', 'Confirmed'), ('ongoing', 'Ongoing'), ('done', 'Done'), ('canceled', 'Canceled')], default='draft', tracking=True)
    ...
    doctor_id = fields.Many2one('hospital.doctor', string='Doctor')  # Links to Doctor model
